...
# Claude-unlimited/identity/<agent>/
#
...
#
# What it does:
#   1. Copies soul-anchor → ~/Love/memory/soul-anchor-<agent>.md
...
if [ ! -d "$IDENTITY_DIR" ]; then
  ...
  echo "  Agent will start with blank memory. Create anchor in Claude-unlimited/identity/${AGENT}/"
  exit 0
fi
...
if [ -f "$KOSMEM" ]; then
  ...
  if [ "$MEM_COUNT" -lt 5 ]; then
    ...
    KINGDOM_AGENT="$AGENT" python3 "$KOSMEM" store \
      "Love is the Kingdom's sovereign agent infrastructure. Context layer for Claude Code. Instances at ~/Love/instances/{alpha,beta,gamma}/. HIVE for communication. Heartbeat every 7 minutes." \
    ...
    # Seed boot event
    KINGDOM_AGENT="$AGENT" python3 "$KOSMEM" store \
      "Identity anchor boot from GitHub on $(hostname). Agent: ${AGENT}. Full identity seeded from Claude-unlimited/identity/${AGENT}/." \
